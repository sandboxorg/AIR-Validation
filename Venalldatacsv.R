...
#RegionCity
RegionCity.data1  <- as.data.frame(sqlQuery(GMAC.data, "select   a.strCityName,AreaScheme,CRESTACode,CRESTAName,AreaCode,AreaName,SubareaCode,SubareaName,PostalCode,'VE' as countrycode,CountryName,Latitude,Longitude  FROM [AIRGeography].[dbo].[tGeography] b 
                                            join [AIRGeography].[dbo].[TblAreaCity] a on a.guidExternal=b.GuidExternal
                                            where b.CountryCode='VE' and remappedgeographysid is NULL and b.AreaScheme='1072'"), stringsAsFactors = TRUE)
...
RegionCity.data1[,c(7,8)]<-0
RegionCity.data2[,c(7,8)]<-0
RegionCity.data3[,c(7,8)]<-0
RegionCity.data4[,c(7,8)]<-0
RegionCity.data1[,c(3,4)]<-0
RegionCity.data2[,c(3,4)]<-0
RegionCity.data3[,c(3,4)]<-0
RegionCity.data4[,c(3,4)]<-0
...
RegionCity.data3[,c(12,13)]<-0 
RegionCity.data4[,c(12,13)]<-0 
...
write.table(upxmydata1, "//qafile2/Leonardo/Feature Data/SouthAmerica/InputFiles/VenezuelaALLData//vencsvalldata/VenRegionCityAlldatalocation.csv", sep=",",quote=FALSE, row.names=FALSE,col.names=FALSE) 
